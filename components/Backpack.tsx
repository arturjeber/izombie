...
export const Backpack = ({ itens, limit }: { itens: any; limit: number }) => {
  ...
  const avaiable =
    limit - itens?.reduce((acc: number, item: any) => acc + item.size * item.quantity, 0);

  return (
    <>
      <div className="mt-4 mb-3 text-xl text-gray-300">
        Food, tools & weapons <span className="text-sm float-end mt-1">{avaiable} spots</span>
      </div>
      <div>
