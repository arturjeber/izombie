...
export const UserBar = ({ titulo, tituloRight, valor, vDecor, cor, st }: UserBarProps) => {
  ...
  const getCor = () => {
    ...
    return valor === 100
      ? '#22c55e' // verde mais claro
      : valor > 80
        ? '#16a34a' // verde escuro
        : valor > 50
          ? '#facc15' // amarelo
          : valor > 40
            ? '#fbbf24' // laranja claro
            : valor > 30
  ...
  return (
    <div className={`w-full ${st}`}>
      ...
      <div className="flex-start flex h-2.5 w-full overflow-hidden rounded-full bg-zinc-700 font-sans text-xs font-medium">
        <div
          style={getSizeCor()}
